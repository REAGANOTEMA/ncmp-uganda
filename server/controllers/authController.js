...
const validateNIN = require('../utils/validateNIN');

exports.register = async (req, res) => {
  try {
    const { full_name, email, password, role, nin, phone, profile_photo } = req.body;

    if (!full_name || !password || !nin) {
      return res.status(400).json({ message: "Full name, password and NIN are required" });
    }
    ...
    const result = await pool.query(
      ...
      [full_name, email || null, hashedPassword, role || "citizen", nin, phone || null, profile_photo || null]
    );
