...
// Combine sample MPs and generate extra MPs dynamically
const ALL_MPs: MPProfile[] = [
  ...
  ...UGANDAN_CONSTITUENCIES.slice(3, 20).map((c, i) => ({
    ...
    activeProjects: `${Math.floor(Math.random() * 50) + 1}`,
          ...
          <div key={mp.id} className="gov-card overflow-hidden group">
            ...
            <div className="px-4 pb-4 -mt-8">
              ...
              <div className="mt-3 flex gap-2">
                <Link
                  to={`/mp-profile/${mp.id}`}
                  className="flex-1 flex items-center justify-center gap-1.5 h-7 rounded-lg text-xs font-medium bg-muted hover:bg-muted/80 transition-colors"
                >
                  <Eye size={12} /> View
                </Link>
                <Link
                  to={`/mp-profile/${mp.id}`}
                  className="flex-1 flex items-center justify-center gap-1.5 h-7 rounded-lg text-xs font-medium transition-all"
